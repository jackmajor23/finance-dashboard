...
function _openStatModal(type, totals) {
  ...
  if (type === 'networth') {
    ...
    S.accounts.filter(a => a.type === 'credit-card').forEach(a => {
      rows.push({ label: `${a.name} <span style="opacity:.5;font-size:11px;">credit card</span>`, value: -a.balance, color: 'neg' });
    });
    // ISAs
...
function _renderEventsTicker(now = new Date()) {
  const wrapper = document.getElementById('eventsTickerWrapper');
  ...
  const chips = events.slice(0, 6).map(ev => {
    ...
    // Use neutral colors (amber/orange) instead of red for urgent
    return `<div style="
  ...
  el.innerHTML = `<div style="display:inline-flex;gap:6px;">${chips}</div>`;
}
...
function _renderMonthlyCashFlow() {
  ...
  if (!el) return;
  if (typeof cashFlowChart !== 'undefined' && cashFlowChart && typeof cashFlowChart.destroy === 'function') cashFlowChart.destroy();
...
function _renderAllocationChart() {
  ...
  if (!el) return;
  if (typeof allocChart !== 'undefined' && allocChart && typeof allocChart.destroy === 'function') allocChart.destroy();
...
function _renderDebtTimeline() {
  ...
  if (!el) return;
  if (typeof debtTimelineChart !== 'undefined' && debtTimelineChart && typeof debtTimelineChart.destroy === 'function') debtTimelineChart.destroy();
...
function _renderNWChart() {
  if (nwChart && typeof nwChart.destroy === 'function') nwChart.destroy();
