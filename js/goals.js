    ...
    return`<div class="goal-card" draggable="true" data-idx="${i}"
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">
        <div style="min-width:0;">
          ...
          <div class="goal-meta" style="display:flex;align-items:center;flex-wrap:wrap;gap:2px;margin-top:2px;">
      ...
      <!-- Stats row -->
      <div class="goal-stats" style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:4px;">
        <span style="color:${col};font-variation-settings:'wght' 600;">${Math.round(p*100)}% saved</span>
        <span style="color:var(--muted);font-size:13px;">
          <span class="val" style="color:var(--text);">${fmt(g.saved)}</span> saved
          &nbsp;·&nbsp;
      ...
      ${sched?`
      <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--muted);margin-top:5px;padding-top:5px;border-top:1px solid var(--border,#f0f0f0);">
      ...
      ${contribs.length?`
      <button onclick="_tlToggle(${i})" style="all:unset;cursor:pointer;margin-top:10px;display:flex;align-items:center;gap:6px;font-size:12px;color:var(--muted);border-top:1px solid var(--border,#eee);padding-top:8px;width:100%;box-sizing:border-box;">
        <span id="tl-arr-${i}" style="font-size:10px;transition:transform .2s;">▶</span>
        ${contribs.length} contribution${contribs.length!==1?'s':''}
        <span style="font-size:11px;opacity:.7;">(${fmt(contribs.reduce((s,c)=>s+c.amount,0))} total)</span>
      </button>
...
function _gdStart(e,i){
  ...
  setTimeout(()=>e.currentTarget.style.opacity='0.45',0);
}
function _gdOver(e){
  e.preventDefault();
...
function _gdLeave(e){ e.currentTarget.classList.remove('goal-drag-over'); }
function _gdDrop(e,i){
  e.preventDefault();
  e.currentTarget.classList.remove('goal-drag-over');
  if(_goalDragSrc===null||_goalDragSrc===i) return;
  const moved=S.goals.splice(_goalDragSrc,1)[0];
  S.goals.splice(i,0,moved);
  _goalDragSrc=null;
  save(); renderGoals();
}
function _gdEnd(e){
  ...
  e.currentTarget.classList.remove('goal-drag-over');
  document.querySelectorAll('.goal-card').forEach(c=>c.classList.remove('goal-drag-over'));
  _goalDragSrc=null;
}
...
function openEditGoal(i){
  ...
  modal.innerHTML=`
    <div style="background:var(--card,#fff);border-radius:18px;padding:24px;width:min(440px,100%);display:flex;flex-direction:column;gap:14px;box-shadow:0 12px 40px rgba(0,0,0,.22);max-height:92vh;overflow-y:auto;">
      ...
      <label style="font-size:13px;color:var(--muted);display:flex;flex-direction:column;gap:4px;">Name
        <input id="_eg-name" class="input" value="${g.name}">
      </label>

      <label style="font-size:13px;color:var(--muted);display:flex;flex-direction:column;gap:4px;">Target Amount
        <input id="_eg-target" class="input" type="number" min="0" value="${g.target}">
      </label>

      <!-- Start + End dates side by side -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <label style="font-size:13px;color:var(--muted);display:flex;flex-direction:column;gap:4px;">
          Start Date
      ...
      <fieldset style="border:1px solid var(--border,#e5e7eb);border-radius:10px;padding:12px 14px;margin:0;">
        <legend style="font-size:12px;color:var(--muted);padding:0 4px;">Log a Contribution</legend>
      ...
      <label style="font-size:13px;color:var(--muted);display:flex;flex-direction:column;gap:4px;">Monthly Savings (reference)
