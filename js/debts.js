...
function renderCreditScores() {
  ...
  if (!S.creditScores.length) {
    grid.innerHTML = `<div class="empty" style="grid-column:1/-1"><div class="ei">◳</div><p>No credit scores tracked yet. Add one below.</p></div>`;
    return;
  }

  grid.innerHTML = S.creditScores.map((cs, i) => {
    const rating = getCreditScoreRating(cs.score);
    const lastUpdatedText = formatLastUpdated(cs.date);
    return `<div class="acc-card" draggable="true" data-cs-index="${i}" ondragstart="dragStartCreditScore(event)" ondragover="dragOverCreditScore(event)" ondrop="dropCreditScore(event)" ondragend="dragEndCreditScore(event)">
      <div class="acc-top">
        <div>
          <div class="acc-name">${cs.provider}</div>
          <div class="acc-type-lbl">Credit score · ${lastUpdatedText}</div>
        </div>
      ...
      <div class="acc-bal val" style="color:${rating.color};font-size:32px;">${cs.score}</div>
      <div class="text-sm text-muted" style="margin-top:2px;color:${rating.color};font-variation-settings:'wght' 600;">${rating.label}</div>
      ${cs.notes ? `<div style="margin-top:8px;" class="text-sm text-muted2">📝 ${cs.notes}</div>` : ''}
    </div>`;
  }).join('');
}
...
function addCreditScore() {
  ...
  ['csProvider', 'csScore', 'csDate', 'csNotes'].forEach(id => document.getElementById(id).value = '');
  renderCreditScores();
}
    ...
    <div class="form-box" style="background:var(--accent-dim);border:1px solid var(--accent);">
      ...
      <div class="grid-2col mb-16" style="gap:12px;">
        ...
        <div>
          ...
          <div class="font-semibold" style="font-size:14px;color:var(--accent);" class="val">${fmt(calcBalance)}</div>
...
function renderDebts() {
  ...
  grid.innerHTML = debts.map((d, i) => {
    ...
    return `<div class="debt-card" draggable="true" data-debt-index="${debtIndex}" ondragstart="dragStartDebt(event)" ondragover="dragOverDebt(event)" ondrop="dropDebt(event)" ondragend="dragEndDebt(event)">
      ...
      ${d.notes ? `<div style="margin-top:8px;" class="text-sm text-muted2" style="background:var(--surface2);border-radius:6px;padding:7px 10px;">📝 ${d.notes}</div>` : ''}
    </div>`;
  }).join('');
...
function addDebt() {
  ...
  const finalShared = shared || personSel === 'shared';
  const person = finalShared ? undefined : parseInt(personSel);
  S.debts.push({ name, type, total, remaining, monthly, rate, start, end, lender, notes, person, shared: finalShared, studentLoanPlan });
...
function saveEditDebt() {
  ...
  d.shared = document.getElementById('ed-shared').checked || personSel === 'shared';
  if (d.shared) delete d.person;
  else d.person = parseInt(personSel, 10) || 0;
